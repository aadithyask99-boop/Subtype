...
**Preset bar:** Preset dropdown (Flex baseline / 5-image 300×600 / Landscape 970×250 / Swipe carousel fluid / Swipe carousel 970×250), Header Html toggles (Logo, YAC, Line2 + text input, Read More), Logo position (6 positions + mobile override), Maintext length, Provider length, Unit Type (IAB Fixed / Responsive / Custom / Original — see "Unit Type System — Four States + Auto-Lock" below), lock badge (click to override auto-detected lock state), Element Order (provider,text / text,provider)
...
### The technique
`overflow-x: auto` + `scroll-snap-type: x mandatory` on `.wrapper`, `scroll-snap-align: start` on each `.hero`, with `.hero` given a `flex-basis` less than 100% so the next card visibly peeks in at the edge — that peek is what signals "swipe for more" without needing dots or arrows (which we can't build — no interactive elements exist in Dianomi's DOM to attach state to, and we can't add any).

### Width/height model for a carousel specifically
Unlike most Dianomi units (which let height grow with content), a carousel's `.wrapper` height should be **fixed** to match the ad slot's reserved height — otherwise the unit would resize as you swipe between cards with different headline lengths, causing layout shift on the publisher's page. To make that safe: image gets a fixed `height` (not just `aspect-ratio`, which can still drift the total height depending on width rounding), and `.maintext` gets `-webkit-line-clamp: 2` so a long headline truncates instead of growing the card. Width still works exactly like any other unit — `.wrapper` width is the ad slot's allocated container width (IAB max-width or Responsive fluid %); the carousel-specific part is each `.hero`'s `flex-basis` as a percentage of that.

### Bug found and fixed — edge-fade tease scrolling with the content instead of staying pinned
First attempt at a "next card is cut off, tease more content" fade used a `.wrapper::after` pseudo-element with a `linear-gradient` background, `position: absolute`, pinned to the right edge. **This visibly scrolled along with the cards instead of staying fixed at the wrapper's edge** — because an absolutely positioned child of a scrolling element is carried along with the scrolled content; its containing block is `.wrapper`, but `.wrapper` is also the thing scrolling.

**Fix — `mask-image` on `.wrapper` itself, not a pseudo-element overlay:**
```css
.wrapper {
...
```
A CSS mask is computed against the element's own box geometry, not its scrolled content, so it stays visually pinned to the edge regardless of scroll position. No extra DOM/pseudo-element needed.

**Known tradeoff:** `::after` faded to solid white (matching `.wrapper`'s background) — a clean scrim. `mask-image` fades the actual pixels to *transparent*, so at the very edge you see through to whatever's behind the ad unit on the publisher's page. Fine-to-better on white/light publisher backgrounds (the unit visually recedes into the page); would show through on a colored/dark publisher background instead of white. Worth checking against a real publisher site, not just Subtype's preview, before shipping.

### Added as two presets in `public/index.html`
Following the existing `CP` object + `#unit-preset` dropdown pattern (same as `flex-baseline`, `list-300x600`, `landscape-970x250`):
- **`carousel-peek`** — "Swipe carousel (fluid, peek cards)": the general-purpose version, fluid width, `.hero { flex: 0 0 72%; max-width: 320px }`, `.wrapper` height auto (no fixed-size assumption). 6 ads, 600px preview width, 280px preview height, Line2 badge ON (sticky, text "Sponsored"), Action OFF.
- **`carousel-970x250`** — "Swipe carousel 970×250 (fixed height, edge fade)": the refined version worked through above — fixed 250px wrapper height, `.hero` fixed at 260×218px, image fixed 120px height, 2-line `-webkit-line-clamp` on `.maintext`, `mask-image` edge fade. 6 ads, 970px preview width, 250px preview height, same header toggle config as the fluid version.

Both presets go through the same "loading a preset is a fresh unit" path as the existing three — `originalCSS`/`customCSS` get captured fresh, lock detection re-runs (`refreshLockState()`), and the unit type UI lands on Custom, exactly like `flex-baseline`/`list-300x600`/`landscape-970x250` already do.

**Not yet added to `dianomi-skill.md`/`generate-css.js`/`refine-css.js`** — these presets are hand-written starting points for the person to pick from the dropdown, not something Gemini can currently generate from a screenshot. If the carousel pattern proves out, folding it into the Gemini-facing knowledge base (as a new lettered pattern, like Pattern J) would be the next step — same process as before: WHY reasoning in skill.md, a reference screenshot as a few-shot example, technique added to refine-css.js.
